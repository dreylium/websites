...
<div
	class="mt-auto bg-clr-DarkViolet bg-boost-mobile bg-cover pb-[5.6rem] pt-[5.9rem] text-center lg:bg-boost-desktop lg:pb-[3.5rem] lg:pt-[3.45rem]"
>
	<h2
		class="mb-[0.85rem] font-PoppinsBold text-[1.85rem] tracking-[-0.055em] text-white lg:mb-[1.4rem] lg:text-[2.5rem] lg:tracking-[-0.025em]"
	>
		Boost your links today
	</h2>
	<a
		href="/"
		class="inline-block rounded-full bg-clr-Cyan px-[2.45rem] py-[0.64em] font-PoppinsBold text-[1.25rem] text-white hover:bg-clr-CyanHover"
		>Get Started</a
	>
</div>

<footer class="bg-clr-footerBg">
	<div class="text-center">
		<!-- Lower -->
		<div
			class="mx-auto grid max-w-[1195px] gap-y-[2.65rem] px-10 pb-[3.35rem] pt-[3rem] text-base lg:grid-cols-[1fr_auto_auto_auto_auto] lg:pt-[4.45rem] lg:text-left"
		>
			<div class="flex justify-center py-[0.4rem] lg:justify-start lg:py-0">
				<a href="/" class="">
			...
			<!-- Footer Navigation -->
			<div class="grid gap-x-[5rem] gap-y-[2.25rem] lg:grid-cols-3 lg:pr-[4.45rem]">
				{#each footerNav as { title, items }}
					<!-- Content here -->
					<section class="flex flex-col gap-y-[0.6rem]">
						<h2 class="mb-3 font-PoppinsBold text-white">{title}</h2>
						{#each Object.entries(items) as [key, value]}
							<a
								href="{value}"
								class="font-PoppinsMedium tracking-[-0.055em] text-clr-GrayishViolet hover:text-clr-Cyan"
								>{key}</a
							>
						{/each}
					</section>
			...
			<div class="py-0.5">
				<div
					class="flex items-center justify-center gap-x-6 lg:px-[0.15rem] [&_a:hover_path]:fill-clr-Cyan [&_path]:fill-white"
				>
					<!-- element a open in new tab when click-->
